...
class VeloController extends Controller
{
    ...
    {
        $zones_repo = $this->getDoctrine()->getRepository('AppBundle:Zone');
        ...
        if ($request->isMethod("POST")) {
            ...
            $em->persist($velo);
            $em->flush();
            $locati=$em->getRepository('AppBundle:Zone')->find($velo->getZone()->getId());
            $locati->setStock(
                ($locati->getStock())+1);
            $em->flush();
    ...
    {
        $velo_repo = $this->getDoctrine()->getRepository('AppBundle:Velo');
    ...
    public function update2Action($idz, Request $request)
    {
        ...
        if ($request->isMethod("POST")) {
            ...
            $velos->setPrix($request->get("prix"));
            $velos->setZone($zone);
            $em->merge($velos);
            $em->flush();
